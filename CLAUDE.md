...
- **2026-08-22** — **Services taxonomy — public vs teammate service pages (LIVE).** Each `/services/{name}` page now renders **by the viewer's role, same URL**: public/clients see a "what we do" page; crew/office see a "how we do it" training page. New **`bos_services`** module — `hook_entity_view_mode_alter` switches the canonical full-page render to the **`teammate_view`** view mode for internal roles (`teammates`, `supervisor`, `administration`, `site_assistant`, `site_admin`, `administrator`), else **`full`** (public). **Fields:** the old ambiguous **`field_description` "Crew Description"** (its help text always said teammate-facing) relabeled **"Crew / Training Description"** = the teammate body; new **`field_public_description`** (`text_long`, shared storage with material_types/backflow_device_types/backflow_uses; plain textarea widget) = the public body. Content consolidated into the public field with correct precedence — the **core taxonomy `description`** base field, when present, is the intentional marketing copy and **wins** (the 5 newer terms: both Lighting bundles, Lighting parent, Backflow, Winter Pruning); otherwise the old Crew Description is the source; the crew text stays as training. **`full` display** = banner image(s) (`field_banner_image`, multi-value) + iconic image + subtitle + public description; **`teammate_view` display** = training body + department + service code (no images — deliberate). **Form** tidied: core `description` **retired from the form** (data left intact), `field_public_description` in the **Public View** tab, new **Teammate View** tab holding the training body. Built via idempotent entity-API scripts (`setup_services_public_description.php`, `build_services_view_modes.php`, `build_services_form_layout.php`) — **no cim**; module is one `hook_entity_view_mode_alter`. Training field is **not rendered at all** for anon (no public leakage — verified). Deployed dev → verified → live (safety dump `~/pre-services-split-20260822.sql.gz.gz`; rsync module + scp scripts + `drush en` + run 3 scripts + `cr`; no cim/DB migration). Verified live: `/services/landscaping` → anon gets banner+public, teammate gets training; form shows both tabs, no core Description. ⚠ **SOP flagged** `CRW-SVC-TRN-001` (per-service crew training on the Services page) — logged in the new **Pending SOPs backlog** in `SOPs/SOP-AUTHORING-WORKFLOW.md`; Chat authors. Docs: this changelog.
