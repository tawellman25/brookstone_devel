...
- **2026-08-03** — **Equipment Type exposed filter on the equipment admin view (LIVE).** Added an exposed **"Equipment Type"** (`field_equipment_type`) dropdown filter to the `equipment` view (page_1 = `/admin/operations/equipment`) so staff can filter the list by type (Skid-Steer, Mini Excavator, …). Cloned from the view's existing exposed **Status** filter (`taxonomy_index_tid`, `vid=equipment_types`, `type=select`); `page_1` inherits the default display's filters, so it's added to default. Idempotent script `add_equipment_type_filter.php` (active-config edit on dev+live — not cim, the view is drifted). Verified present on both.
