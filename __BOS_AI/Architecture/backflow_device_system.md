...
| `field_short_description` | text_long | Teaser / summary copy (added 2026-09-26). The landing view card shows this or a 300-char trim of the public description. Reuses the shared storage (brookstone_tags/growth_zone). |
| `field_teammate_description` | text_long | Crew-facing copy (added 2026-09-26), the type-axis counterpart to the services `field_service_crew_desc`. Reuses the shared storage (also on `backflow_uses` + many). |
| `field_banner_images` | image, multi | Top banner image(s) for the type page (added 2026-09-26). Standard `image_image` upload widget. Reuses the shared storage (equipment_types). |
| `field_meta_tags` | metatag | Per-term SEO override (added 2026-09-26). The metatag module feeds these into the term's meta tags automatically (no hook), same as the services vocab. `metatag_firehose` widget. |

> **Content fields added 2026-09-26** — the four fields above (`field_short_description`, `field_teammate_description`, `field_banner_images`, `field_meta_tags`) give the device-type vocab the same content tier as the services vocab. All four reuse **existing** `taxonomy_term` storages (instances + form widgets only). Setup: `web/scripts/setup_backflow_device_type_content_fields.php` (idempotent).

> **Audience view-mode tiers + rotating hero (2026-09-26, `bos_backflow_types` module)** — the device-type term page follows the BOS audience pattern (§ui_patterns.md), same as the services vocab: one `/services/backflow-prevention/{type}` URL, body chosen by role — office/admin → **admin_view** (all fields), crew → **teammate_view** (`field_teammate_description`), public/clients → **full** (`field_public_description`). `field_banner_images` renders for public viewers as a **full-bleed rotating hero** in the theme's top `hero` region (`bo_hero_banner` theme, `max_2600x2600` derivatives, `field_short_description` as the subtitle); the page-title block is hidden for public when a banner shows (hero carries the H1), and the banner is hidden from the public body. `user.roles` cache context is added to the term view + page (mandatory — role picks the view mode). Displays built by `web/scripts/setup_backflow_type_view_modes.php` (idempotent, entity-API so the view-mode displays register); `field_meta_tags` stays hidden on displays (SEO via the form). This also gave the term canonical page its public description, which the pre-module default display didn't render.
