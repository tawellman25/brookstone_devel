...
> **Content fields added 2026-09-26** — the four fields above (`field_short_description`, `field_teammate_description`, `field_banner_images`, `field_meta_tags`) give the device-type vocab the same content tier as the services vocab. All four reuse **existing** `taxonomy_term` storages (instances + form widgets only). Rendering of the new fields on the term/landing pages is a separate display decision — the fields are editable now; where they show is not yet wired beyond what the metatag field does automatically. Setup: `web/scripts/setup_backflow_device_type_content_fields.php` (idempotent).
