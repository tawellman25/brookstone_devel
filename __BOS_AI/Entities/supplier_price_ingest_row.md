...
- `field_row_status` (list_string) — `dry_run` / `committed` / `discovery_pending` / `discovery_resolved` / `rejected` / `error`. Lifecycle:
  ...
  - `committed` — set per-row by `IngestCommitter` (Phase 3.6) when `PriceSyncService::ingestRow()` returns a non-error outcome (`applied` / `flagged_high` / `auto_created`). **Phase 3.6: catalog mutations and `material_price_history` writes are real now.** The row's `field_resolution_action` is also stamped at this point: `created_link` for `auto_created` outcomes; `updated_link` for `applied` and `flagged_high` outcomes. Auto-applying tiers (tier_1_mfr / tier_2_supplier_sku / tier_3_fuzzy_high) are touched here. **Phase 3.7: also reached via the Fuzzy Match Review "Confirm Match" and "Override Match" operations** when an office reviewer commits a previously-pending fuzzy row.
  - `discovery_pending` — **set by IngestCommitter (Phase 3.7)** when a committed batch contains rows tagged `field_match_tier IN ('discovery', 'tier_3_fuzzy_med')`. The committer's `routeRemainingRowsToDiscovery()` step transitions these rows from `dry_run` after the auto-applying commit loop finishes. Both the Discovery Queue view and the Fuzzy Match Review view filter on this status; they slice on `field_match_tier` to differentiate the two queues.
  - `discovery_resolved` — **set by the Discovery Queue per-row operations (Phase 3.7)** when an office reviewer resolves a discovery row. The matching `field_resolution_action` records WHICH resolution path was taken: `created_new_material_and_link`, `linked_to_existing_material`, or `marked_as_replacement`. The Fuzzy Match Review Confirm/Override flows skip this state and go directly to `committed` because they're effectively a commit, not a discovery decision.
