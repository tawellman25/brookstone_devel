...
- **What it does:** 15 min after the fetch, it reads the latest block in `~/wex_fetch.log` and **emails `todd@brookstoneoutdoors.com` only on a real failure** — the block isn't from today (cron didn't fire), or it lacks the `fetch-email complete` line (drush crashed, IMAP auth failed, etc.). Every **successful** run — including a day with **no WEX email at all** — prints `WEX fetch-email complete`, so quiet weekend / no-fuel days do **not** alert.
  - **2026-07-13 fix (commit `8abb41d1`):** the no-email path used to `return` *before* the summary line, so days WEX sent nothing (07-12, 07-13) tripped a **false** "import failed" alert even though nothing was wrong. The command now emits the `fetch-email complete` sentinel on the no-messages branch too, and the watcher additionally treats `No UNSEEN WEX messages found` as healthy (belt-and-suspenders). If you ever change the command, keep the invariant: **every successful exit path must print `fetch-email complete`.**
