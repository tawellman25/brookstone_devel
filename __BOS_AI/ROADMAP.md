...
| **Owned-equipment billing — Phase 2 (invoicing)** | `wo_profit` / `wo_*` sign-off / `equipment` | T2 | L | **Phase 1 SHIPPED live 2026-08-03** — owned heavy/small equipment used on a WO shows cost (× Operating Cost/hr) + billable revenue (× "Hourly Work Order Rate") on the **live profit panel** (visibility-only, no invoicing). **Phase 2:** put owned-equipment charges on the **actual invoice** — consolidate the ~32 `wo_*` `get_*_total_rental_fees` helpers into **one shared rental/equipment service** (rented + owned) feeding `field_wo_total` → QuickBooks. Needs per-machine rates entered + validated via Phase 1; careful billing tests. Extend to more equipment types (vehicles/sprayers/trailers) as needed. Owner: visibility-first, then invoice. |
