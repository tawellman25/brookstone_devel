...
**Owner:** Todd · **Repo path:** `__BOS_AI/ROADMAP.md` · **Last reconciled:** 2026-07-03 · **Last updated:** 2026-08-03 (added NEXT: WO cost & profit visibility — Stage 1 live per-WO panel + stored/frozen `field_wo_cost`/`field_wo_profit`; Stage 2 profit-by-service-line dashboard)
...
| **⭐ WO cost & profit visibility (Stage 1: live per-WO panel)** | `wo_sign_off` / `business_setting` / new panel · all 36 WO bundles | T2 | M | Supervisor/admin-only **Cost & Profit** per work order, live "as things happen." Revenue already computed (`field_wo_total` + component fields). Cost from **existing snapshots** — materials `wo_material_list_item.field_material_cost`, chemicals `wo_chemicals_used.field_chemical_cost`, rentals `field_receipt_total_cost`, dump `field_dump_fee_total` — plus **labor = `field_total_time` × a new blended labor-cost/hr rate in `business_setting`** (interim until real TimeTrax punch-cost — see TimeTrax row above). **Store + freeze `field_wo_cost` / `field_wo_profit` at completion** (in the `wo_sign_off`/`wo_billing_recalc` flow that already freezes billing totals) so historical margins stay rate-accurate + no future backfill; **live compute-on-read panel** for in-progress WOs. Metric = **job-level gross margin** (revenue − direct costs; excludes fuel/overhead/depreciation — be explicit in the UI). Role-gated to supervisor/administration/site_admin/administrator; crew + client never see it. **Stage 2 (LATER):** profit-by-service-line **dashboard** on the stored fields — group by service/bundle (+ crew/season/property) to answer "are fertilizing WOs actually profitable overall?", same pattern as Daily Recap / Teammate Ops. |
