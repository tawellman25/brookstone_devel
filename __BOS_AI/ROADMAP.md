...
**Owner:** Todd · **Repo path:** `__BOS_AI/ROADMAP.md` · **Last reconciled:** 2026-07-03 · **Last updated:** 2026-09-07 (Sprinkler System Type — the pump-fee charge driver — now recorded on winterizing + start-up WOs: pre-fill from property, crew-override billing, freeze at completion, write-back; 9,831 historical WOs backfilled)
...
| **Contact opt-in/consent model + consent log — SHIPPED LIVE 2026-09-05** | `contacts` / `consent_log` / `bos_consent_log` | T1 | M | Person-vs-account split: Contact = person + opt-ins only (no access, nothing sensitive); User+`customer_profile` keep all secure data; `field_primary_contact_ref` links them. Split opt-in booleans (marketing/service × email/sms) + consent_updated/source cache on Contact; append-only `consent_log` ECK entity auto-written on every flag change (immutable; can't drift from the flags). Backfilled the link gap **651→37** (605 contacts created); migrated legacy User consent → Contact (service only); retired the 3 User consent fields. Discovery: JSON:API read-only + anon can't read Property/WO/Contact; all files public-scheme; **no customer email is sent today** (send-time consent = greenfield). **Follow-ups:** office reviews 37 unlinked profiles + 2 unmigrated consents; intake forms write opt-in to Contact; send-time consent check when a customer mail path is built; optional office View over `consent_log`. Docs: `docs/contact-consent-plan.md`, `docs/contact-model-discovery.md`. |
| **Customer-portal waitlist — SHIPPED LIVE 2026-09-06** | `bos_portal_waitlist` | T2 | S | Two-column login page: crew login (unchanged) + "Coming for customers" waitlist panel. Captures portal interest (product-validation signal) + opt-ins onto Contact (consent home) via `bos_consent_log`; not an account signup. reCAPTCHA + flood, `?c=portal26`, multi-signal contact matching, opt-out guard, fabricated-email override. **Follow-ups:** ⚠ SOP for office lead handling; no web analytics (funnel unmeasured — add before paid campaign); the "accounts are open" email needs the send-time consent check (not built); curate the 4 portal-promise bullets. Signup volume decides whether the **customer portal itself** (a winter build) is worth it. Docs: `Modules/bos_portal_waitlist.md`. |
| **Analytics — GTM + conversion events — SHIPPED LIVE 2026-09-06** | `bos_analytics` | T2 | S | First analytics on BOS. GTM container (single tag host) on every page incl. login; container id in settings.php (env-specific, dev untracked). No-PII dataLayer events on public-form success (`winterize_submit`/`cleanup_submit`/`portal_waitlist_submit`, `{event,campaign}` only); live forms unaltered; cannot break a form. **Todd's remaining steps (in GTM web UI, `GTM-WC8F8PCN`):** build the GA4 (`G-BGR1SF7LTM`), Meta Pixel, and Google Ads conversion tags (Pixel ID + Ads conversion ID needed there, not in code); trigger Lead/conversion on `winterize_submit`. Then browser verification (GTM Preview / Pixel Helper / GA4 DebugView / end-to-end `?c=fb26`). Needed before the ~$1,200 paid campaign. Docs: `Modules/bos_analytics.md`. |
| **Sitewide footer + marketing pages — SHIPPED LIVE 2026-09-12** | `brookstone_olivero` (+ `bos_service_request`, `bos_homepage`) | T2 | M | Role-gated sitewide footer (anonymous + client; never on the crew/office admin theme) in Olivero's `footer_top`/`footer_bottom` regions — fielded Company/NAP block, Services + Company menus, plain-text Service Area (local-search signal), a **date-driven seasonal promo** (`footer_promo` view, 1-hour max-age, renders nothing when inactive), and a dynamic-year legal bar with Privacy link. Built `56e9be5b`, robustness/500-recovery `27decdb1`. **Layout restyle `a211e866`** ported the marketing footer reference (full-bleed — removed Olivero's 90px left border, no `page.html.twig` copy; four-column grid on `.region--footer_top__inner`; one-row legal bar; empty-promo collapse via `:not(:has())`; WCAG-AA hours ~7.8:1; logo; hover/focus affordances; reuses the theme `--max-width`). **Root-cause lesson:** the original CSS targeted the wrong Olivero region classes (single- vs double-hyphen + `__inner`) so every rule was inert — verified against rendered HTML per gotcha #1. Theme layer only; zero config, zero content. Sibling marketing pages also shipped this session: **/about** (`885fb3b8`), **/privacy** (`ebd3a49d`), **/contact** rebuilt as a real BOS lead with routing + attribution (`bfaeb13a`). **Follow-ups:** the orange sticky/mobile-nav header toggle is separate header chrome (from the `bo-brand.css` header-height work) — its own task if it needs adjusting; Facebook `sameAs` for JSON-LD; full public-site rebuild remains the separate Jan–Feb 2027 project. Docs: CLAUDE.md footer entries. |
...
| Item | Area | Tier | Effort | Notes |
|---|---|---|---|---|
| **HOA / Common Area — Phase 2 boundary-input UX + Phase 3 (public pages)** | `properties` (`hoa` bundle) / `bos_hoa` | T2 | M | **Phase 1 SHIPPED** (`653d661f`): `hoa` bundle (superset of property), `/admin/properties` Type + Discounted filters; **14 real HOAs converted** in place (reversible, all history intact). **Phase 2 ENGINE SHIPPED live 2026-09-07** (`e7bb282e`, `bos_hoa`): PHP ray-cast point-in-polygon (not MariaDB ST_Contains) + geofield bbox pre-filter auto-links homes inside a discounted HOA boundary via new `field_hoa` and sets `field_hoa_contracted` → $60 discount; hooks re-sync on HOA/home save; `drush bos:hoa:sync-members`. 9/9 tests. **Phase 2 boundary-input DONE** (`3acbda0e`): a **Google Maps polygon-draw widget** (`hoa_boundary_map`, reuses the existing geofield_map Google key — NOT leaflet_widget, which is dev-only) lets office draw the boundary on the HOA edit page (WKT round-trip). **Bear Creek pilot LIVE:** 6 member homes auto-flagged for the $60 discount. **To activate an HOA:** (a) check "Discounted" on the HOA record + (b) draw its boundary → scan auto-flags member homes. **Phase 2 fully shipped.** **Phase 3 SHIPPED live 2026-09-07** (`1d3c0c8e`): public showcase on the HOA's **canonical property page** — one URL per record, role-aware (Services pattern; anon/clients get a `public` view mode with safe fields + `property_photos_public` gallery, staff get the full page; property-internal blocks gated off for the public; residential member homes stay 403 to anon). _(An initial separate-`/hoa/{slug}`-route build was retired same-day per direction to match how all properties work.)_ **HOA epic (Phases 1–3) COMPLETE.** Remaining = **data entry** (office: fill public descriptions + stage public photos + draw precise boundaries + check "Discounted" per HOA) and optional polish (a public HOA index/listing; refine Bear Creek's rough pilot boundary). Docs: CLAUDE.md 2026-09-07. |
| **Invoice-photo → material-list import (vision extraction)** | `wo_material_list_management` | T2 | M | **Extension of the shipped Import Items pipeline** (`Modules/wo_material_list_import.md`; parse→match→preview→import→learn, round-trip-fixed 2026-08-28). Add a **photo/PDF** intake in front: upload/snap a supplier invoice → **gpt-4o vision** (already wired: `ai` + `ai_provider_openai`, `ai.settings` `chat_with_image_vision` = openai/gpt-4o, key `openai_api_key`) extracts structured rows (item # · description · qty · unit price) → **drop into the existing preview** (match + per-row material autocomplete + supplier "learn"). **The hard 90% is already built** — only the image→rows step is new. **Mandatory preview stays** (photos are messy: glare, angle, multi-page). Design/build started 2026-08-28: `InvoiceVisionExtractor` service (image → rows JSON via the AI provider) + a "Photo/PDF" option on the import form. Verify the `openai_api_key` key has a live value + watch token cost per invoice. |
...
| **Owned-equipment billing — extend to more types** | `wo_rental_equipment_ui` / `equipment` | T2 | M | **Phase 1 + 2 SHIPPED live 2026-08-03.** Phase 1 = owned heavy/small equipment cost + billable revenue on the live profit panel. **Phase 2 (invoicing) DONE** via `wo_rental_equipment_ui`: owned + Billable machine → `field_hourly_rate` auto-filled from `equipment.field_rate` on save → the **existing** rental billing charges it → `field_wo_total` → invoice (no 32-module refactor needed — feeding the existing `COALESCE(receipt, hourly×hours)` the rate sufficed). Equipment section now shows cost + charge cards. **Remaining:** currently only **heavy_equipment + small_engine** carry the rate/billable/operating-cost fields; add them to **vehicles / sprayers / trailers** (+ `field_equipment_used` target list) if those should be billable too. Office must set rate + check Billable per machine. |
| **WO cost & profit — Stage 2: profit-by-service-line dashboard** | `wo_profit` | T2 | M | **Stage 1 SHIPPED live 2026-08-03** (`wo_profit` module — per-WO live Cost & Profit panel + stored/frozen `field_wo_cost`/`field_wo_profit`, blended **$27** labor rate; see Shipped archive). **Stage 2:** a supervisor **dashboard** aggregating the stored profit **by service/bundle** (+ crew / season / property) → "are fertilizing WOs actually profitable overall?", same pattern as Daily Recap / Teammate Ops. Data accrues from Stage 1's frozen fields on new completions; decide a **historical strategy** for pre-existing WOs (one-time re-save backfill at current rate, or compute-on-read into the dashboard). |
...
| Customer-portal waitlist SOP | SOPs / `OFF-POR-WTL-001` | T3 | S | **⚠ SOP-Needed (flagged 2026-09-06).** Office workflow for managing the login-page waitlist (`bos_portal_waitlist`): finding waitlist Contacts (filter `field_portal_interest`), reading the lead note, what consent each submission grants, and the hard rule that **no customer email may go out until the send-time consent check exists** (incl. the "accounts are open" notice → filter opt-in flags + a real external address). In the SOP-Needed backlog (`SOPs/SOP-AUTHORING-WORKFLOW.md`); Chat authors, Code installs. |
| **SEO standardization — enable Metatag + install `schema_metatag`** | SEO / `metatag` · `schema_metatag` · `bos_homepage` | T2 | M | **The proper structured-data + meta-tags layer for the growing public site.** Today: Metatag is downloaded (`^1.22`) but **entirely disabled**; `schema_metatag` is **not installed**; structured data is **hand-rolled** in two places — `bos_homepage` (LocalBusiness JSON-LD, front only) and the About page (Organization JSON-LD via a `/about`-gated `hook_page_attachments`, shipped 2026-09-09). Do it as **one deliberate pass**: `composer require drupal/schema_metatag`, enable Metatag core + schema_metatag, configure Organization / LocalBusiness / WebPage / BreadcrumbList as managed config, then **migrate BOTH hand-rolled JSON-LD blocks into it** (retire the `bos_homepage` block + the About hook) so there is a **single, UI-editable** source the office can maintain without deploys (titles/descriptions/OG images per page too). Worth it once there are several public pages (home, winterize, fall-cleanup, about, credentials, services). **Watch:** new config surface under the "intentionally-drifted" regime (partial-cim discipline); avoid duplicate structured data during the cutover. Not in the tier list yet — Tier-2 SEO contrib, justify on adoption. |
...
| Item | Area | Tier | Effort | Notes |
|---|---|---|---|---|
| **Snow contract (residential) — P1–P4 SHIPPED LIVE** | `contracts:snow_removal` / `contract_snow` / `wo_snow_removal` | T2 | L | Tiered per-plowing rates (0–2/2–4/4–6/6+) + per-push, snow-trigger vocab (+ Other free-text), per-customer salt/mag ice rates (+ Business-Settings defaults) w/ per-visit Bag/Pound max, 2-page Snow Removal Service Agreement PDF (`contract_snow.agreement_pdf`, entity_print+dompdf+QR, brand tokens), the **workflow toolbar** (Preview / Mark Sent / Upload Signed / Activate) with signed-PDF upload + **hard version-lock** (Executed/Active freezes pricing+terms), and **P4 depth-tiered billing** — `snow_levels` vocab standardized to 0-2"/2-4"/4-6"/6"+/Icy w/ `field_snow_depth_tier`; `wo_snow_removal.get_snow_plow_rate_for_wo()` maps recorded depth → contract tier rate, falling back to the flat per-push rate. Deployed via entity-API setup scripts + rsync. Docs: `Modules/contract_snow.md`, `Modules/wo_bundle_modules.md`. **Follow-ups:** snow-trigger term descriptions; office fills real tier rates on live contracts. |
...
**Winterizing pricing — $95 base + auto-applied promo discounts — SHIPPED live 2026-09-07** (`wo_sprinkler_winterizing`, `bos_service_request`). Base fee $90→**$95**; 4 discount amounts on Business Settings (contract/auto-list −$5, >4 services −$10, new-customer 4+ homes −$15, HOA −$35). Applied automatically at sign-off — **single largest wins, no stacking** — recorded on the WO (`field_winterize_discount`/`_reason`) and subtracted from the total. HOA = property flag (`field_hoa_contracted`, new, + `field_neighborhood`); Bear Creek signups auto-set it (4 existing backfilled). New-customer = office checkbox on the WO (that year only). `/winterize` shows $95, `/winterize/bear-creek` $60, both from Business Settings. Net: new $95 · returning $90 · 4+ svc $85 · new-4-homes $80 · HOA $60. 17/17 dev tests. Winterizing only. Commits `22a9b6bd`/`82c0245a`. **⚠ SOP** owed (office: HOA checkbox on property, New-Customer checkbox on WO). Docs: `Modules/wo_bundle_modules.md`. Partially delivers deferred_work #27 (the contracted-rate mechanism is built; the Jan rate-card decision is now just tuning the amounts).

**Sprinkler System Type on winterizing + start-up WOs — SHIPPED live 2026-09-07** (`wo_sprinkler_winterizing`, `wo_sprinkler_start_up`). The winterizing/start-up charge is driven by **System Type** (Domestic = base only; Dirty/Duel/Well add the **$25 pump fee**), but the WO never stored it — billing read the **property's** `field_system_type` live at sign-off, so it was invisible to crews, unqueryable per-WO, and any later property edit silently re-priced old WOs. Added **`field_system_type`** (shared `work_order.field_system_type` storage → `sprinkler_system_types`, radios) to both bundles and made the WO the source of truth: **pre-fill** from the property's primary sprinkler system on create; **bill off the WO's own value** (a crew correction on site wins) with property fallback; **freeze** the effective value onto the WO at completion (past WOs never re-price); **Option A write-back** — on completion push the WO's System Type back onto the property's single sprinkler system so the property stays current. Multi-system properties (rare) keep the per-system property sum. Winterizing base $90 / start-up base $80 / shared $25 pump; TID map 13=Domestic, 15=Dirty, 16=Duel, 30742=Well. 7/7 reversible dev tests each. **Backfilled 9,831 historical WOs** via **direct DB insert** (no entity save → no billing recompute; frozen totals untouched): 6,984 Domestic / 2,360 Dirty / 284 Duel / 203 Well; 301 had no system type on file (self-heals as future WOs complete + write back). Deploy: rsync 2 modules + 2 setup scripts + `cr` (no cim/DB migration). Now a one-field query — e.g. 2025 completed winterizing = **205 pump / 405 domestic**. Setup: `web/scripts/setup_wo_winterizing_system_type.php`, `setup_wo_start_up_system_type.php`; backfill `backfill_wo_system_type.php`. Commits `6e92b1de` / `f29f4a74` / `663fc1bb`. Docs: `Modules/wo_bundle_modules.md`.

**Winterizing schedule carry-forward — SHIPPED live 2026-08-21** (`bos_scheduling` — `bos:winterize:plan` / `bos:winterize:apply`). Proposes next season's `sprinkler_winterizing` schedule from prior-season history (recency-wins date via nth-weekday-of-month; tech = actual signer; route order = sign-off-driven, dense per date+tech; proximity-fill for new customers) and applies the reviewed CSV as real `scheduling` records — creates ONLY schedules + flips `field_scheduled`, never touches WO/status/Contract. Idempotent; `--actor` required (uid 1 needs `--allow-superuser`); soft proposals (`field_scheduled_firm=FALSE`, no notify). Extracted the shared `ScheduleWriter` service (bulk scheduler refactored onto it, behavior-identical). **Live: 457 records applied** (432 auto+tech / 25 unassigned / 2 Sunday-review held), verifier 8/8, idempotent re-run 0/459; safety dump taken. Office follow-up: confirm/assign the 25 unassigned + add the 2 Sunday rows from `~/tmp/winterize_plan_2026_REVIEW.csv`. Re-runnable each season; probe re-checks the signals against new data first. Docs: `Modules/winterize_carry_forward.md`.

**WO cost & profit visibility Stage 1 — SHIPPED live 2026-08-03** (`wo_profit` module, enabled on live). Supervisor/admin-only per-WO cost & gross-profit: blended labor cost/hr in `business_setting` (**$27**), `field_wo_cost`/`field_wo_profit` on all 35 real WO bundles, `WoProfitCalculator` (labor = hrs × blended rate; materials = Σ `field_subtotal` cost basis; chemicals/rentals/dump pass-through; profit = `field_wo_total` − cost = job-level gross margin), generic `entity_presave` (weight 100) freezing at Complete (1097), role-gated live panel (`view wo cost profit`). Verified live (WO 9882 → cost $1,810.13 / profit $1,837.83 / 50.4%; panel shows for admin, hidden from anon). **Stage 2 (dashboard) remains in NEXT.** Docs: `Modules/wo_profit.md`.
