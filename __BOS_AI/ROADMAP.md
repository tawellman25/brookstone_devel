...
| Item | Area | Tier | Effort | Notes |
|---|---|---|---|---|
| **HOA / Common Area — Phase 2 (GPS auto-flag) + Phase 3 (public pages)** | `properties` (`hoa` bundle) | T2 | M | **Phase 1 SHIPPED live 2026-09-07** (`653d661f`): `hoa` bundle on `properties` (reuses property page/map/gallery/WOs) with `field_boundary` (GPS polygon), discounted flag, public description, service start/end; `/admin/properties` Type + Discounted filters. **Phase 2:** draw/store HOA polygons (upgrade `field_boundary` to a map-draw widget) + **point-in-polygon** (MariaDB `ST_Contains`) to auto-set `field_hoa_contracted` on residential homes inside a discounted HOA → winterizing HOA discount ($60) with no manual checkbox. **Phase 3:** public HOA showcase pages — bundle-specific **public** view mode + route (residential stays private), before/after photo gallery (reuse `bos_property_gallery`), and the service timeframe ("drive by and see the difference"). Decision locked (Todd): GPS auto-detect, foundation-first. Docs: CLAUDE.md 2026-09-07. |
