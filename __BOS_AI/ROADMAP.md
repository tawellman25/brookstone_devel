...
**Teammate "Time on Jobs" self-service hours — SHIPPED 2026-07-05.** New `bos_teammate_hours` module (Block `teammate_time_on_jobs` on `/teammates`, `brookstone_olivero`). A teammate sees **their own** `wo_time_clock` hours for a calendar week (Sun–Sat), grouped by day with per-day + week totals; each entry links its WO + property. Self-only (reads `current_user`), **no GPS**, **no dollar figures** (WO clocked hours ≠ billable ≠ compensable). Open entries flagged + excluded from totals; prev/next week nav. Deployed via `drush en` (config/install block); verified live (real crew week, 15.63 hrs). Docs: `bos_teammate_hours.md`. **Follow-on (deferred):** a supervisor/office surface to view *any* teammate's hours (the crew view is self-only); GPS/variance stays in the ops hub.
