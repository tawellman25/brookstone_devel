...
/**
 * Controller for creating "Aerating" work orders.
 */
class AeratingWorkOrderController extends ControllerBase {

  /**
   * Creates a "Aerating" work order for a given contract.
   *
   ...
   */
  public function createAeratingWorkOrder($contract_id) {
    ...
    // Load the referenced Aerating of Lawn entity.
    ...
    if (!$aerating_section) {
      $messenger->addError('Referenced Aerating of Lawn entity not found.');
      return;
    }

    // Check if a work order already exists.
    if (!$aerating_section->get('field_work_order')->isEmpty()) {
      $messenger->addError("A Aerating of Lawn Work Order already exists for this Contract #$contract_id.");
      return;
    }

    // Get the value of the 'field_estimate' field from the Aerating of Lawn entity & Extract the last number.
    ...
    // Create a new Aerating of Lawn Work Order.
    ...
    // Implement the logic to create the Aerating of Lawn work order.
