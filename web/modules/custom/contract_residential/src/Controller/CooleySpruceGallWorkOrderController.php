...
/**
 * Controller for creating "Dormant Oil" work orders.
 */
class CooleySpruceGallWorkOrderController extends ControllerBase {

  /**
   * Creates a "Dormant Oil" work order for a given contract.
   *
   ...
   */
  public function createCooleySpruceGallWorkOrder($contract_id) {
    ...
    // Load the referenced Dormant Oil entity.
    $cooley_spruce_gall_id = $contract->get('field_cooley_spruce_gall_treatme')->target_id;
    ...
    if (!$cooley_spruce_gall_section) {
      $messenger->addError('Referenced Dormant Oil Section entity not found.');
      return;
    }

    // Check if a work order already exists.
    if (!$cooley_spruce_gall_section->get('field_work_order')->isEmpty()) {
      $messenger->addError("A Dormant Oil Work Order already exists for this Contract #$contract_id.");
      return;
    }

    // Get the value of the 'field_estimate' field from the Dormant Oil entity & Extract the last number.
    ...
    // Get the value of the 'field_cooley_spruce_gall_season' field from the Pre-emergent entity.
    $contract_season = $cooley_spruce_gall_section->get('field_cooley_spruce_gall_season')->target_id;
    ...
    $work_order = $entity_type_manager->getStorage('work_order')->create([
      ...
      'field_work_todo_description' => [
        'value' => "<p>" . date('Y') . " - Dormant Oil as described</p>",
        'format' => 'full_html',
      ],
    ]);
    $work_order->save();
    ...
    $cooley_spruce_gall_section->set('field_work_order', $work_order_id);
    $cooley_spruce_gall_section->save();

    // Display success message.
    $messenger->addMessage($this->t("Dormant Oil work order created for Contract #$contract_id."));

    // Implement the logic to create the Dormant Oil work order.
