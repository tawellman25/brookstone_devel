...
/**
 * Controller for creating "Deciduous Bore" work orders.
 */
class DeciduousBoreWorkOrderController extends ControllerBase {

  /**
   * Creates a "Deciduous Bore" work order for a given contract.
   *
   ...
   */
  public function createDeciduousBoreWorkOrder($contract_id) {
    ...
    // Load the referenced Deciduous Bore entity.
    $deciduous_bore_id = $contract->get('field_deciduous_bore')->target_id;
    $deciduous_bore_section = $entity_type_manager->getStorage('contract_sections')->load($deciduous_bore_id);
    if (!$deciduous_bore_section) {
      $messenger->addError('Referenced Deciduous Bore Section entity not found.');
      return;
    }

    // Check if a work order already exists.
    if (!$deciduous_bore_section->get('field_work_order')->isEmpty()) {
      $messenger->addError("A Deciduous Bore Work Order already exists for this Contract #$contract_id.");
      return;
    }

    // Get the value of the 'field_estimate' field from the Deciduous Bore entity & Extract the last number.
    ...
    // Get the value of the 'field_deciduous_bore_season' field from the Pre-emergent entity.
    $contract_season = $deciduous_bore_section->get('field_deciduous_bore_season')->target_id;
    ...
    $work_order = $entity_type_manager->getStorage('work_order')->create([
      ...
      'field_work_todo_description' => [
        'value' => "<p>" . date('Y') . " - Deciduous Bore as described</p>",
        'format' => 'full_html',
      ],
    ]);
    $work_order->save();
    ...
    $deciduous_bore_section->set('field_work_order', $work_order_id);
    $deciduous_bore_section->save();

    // Display success message.
    $messenger->addMessage($this->t("Deciduous Bore work order created for Contract #$contract_id."));

    // Implement the logic to create the Deciduous Bore work order.
