...
/**
 * Controller for creating "Fertilizing" work orders.
 */
class FertilizingEarlySpringWorkOrderController extends ControllerBase {
   ...
   */
  public function createFertilizingSpringWorkOrder($contract_id) {
    ...
    $fertilizing_early_spring_id = $contract->get('field_lawn_fertilizing_broadleaf')->target_id;
    $fertilizing_early_spring_section = $entity_type_manager->getStorage('contract_sections')->load($fertilizing_early_spring_id);
    if (!$fertilizing_early_spring_section) {
      $messenger->addError('Referenced Fertilizing Section entity not found.');
      return;
    }

    // Check if a work order already exists.
    if (!$fertilizing_early_spring_section->get('field_4th_work_order')->isEmpty()) {
      $messenger->addError("A Early Spring Fertilizing Work Order already exists for this Contract #$contract_id.");
      return;
    }

    // Get the value of the 'field_estimate' field from the Fertilizing entity & Extract the last number.
    $estimate_text = $fertilizing_early_spring_section->get('field_estimate')->value;
    ...
    if ($result) {
      ...
      if ($property_fertilizing_info && !$property_fertilizing_info->get('field_turf_pre_emergent_property')->value == 1) {
        ...
        $fertilizing_season = 'pre'; // Default value if the field is empty or doesn't exist
      }
    ...
    $work_order = $entity_type_manager->getStorage('work_order')->create([
      ...
      'field_fertilizing_season' => $fertilizing_season,
      'field_current_turf_sq_footage' => $current_turf_sq_footage,
      'field_work_todo_description' => [
        'value' => "<p>" . date('Y') . " - <strong>Early Spring</strong> Fertilizing as described</p>",
        'format' => 'full_html',
      ],
    ]);
    $work_order->save();
    ...
    // Set the value of the Contract Section's 'field_4th_work_order' reference field to the ID of the created work_order.
    $fertilizing_early_spring_section->set('field_4th_work_order', $work_order_id);
    $fertilizing_early_spring_section->save();

    // Display success message.
    $messenger->addMessage($this->t("<strong>Early Spring</strong> Fertilizing work order created for Contract #$contract_id."));

    // Implement the logic to create the Spring Fertilizing work order.
