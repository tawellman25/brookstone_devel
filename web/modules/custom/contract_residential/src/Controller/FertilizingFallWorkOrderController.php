 ...
 */
class FertilizingFallWorkOrderController extends ControllerBase {
   ...
   */
  public function createFertilizingFallWorkOrder($contract_id) {
    ...
    $fertilizing_fall_id = $contract->get('field_lawn_fertilizing_broadleaf')->target_id;
    $fertilizing_fall_section = $entity_type_manager->getStorage('contract_sections')->load($fertilizing_fall_id);
    if (!$fertilizing_fall_section) {
      $messenger->addError('Referenced Fertilizing Section entity not found.');
      return;
    }

    // Check if a work order already exists.
    if (!$fertilizing_fall_section->get('field_3rd_work_order')->isEmpty()) {
      $messenger->addError("A Fall Fertilizing Work Order already exists for this Contract #$contract_id.");
      return;
    }

    // Get the value of the 'field_estimate' field from the Fertilizing entity & Extract the last number.
    $estimate_text = $fertilizing_fall_section->get('field_estimate')->value;
    ...
    $work_order = $entity_type_manager->getStorage('work_order')->create([
      ...
      'field_fertilizing_season' => 'fall',
      'field_current_turf_sq_footage' => $current_turf_sq_footage,
      'field_work_todo_description' => [
        'value' => "<p>" . date('Y') . " - <strong>Fall</strong> Fertilizing as described</p>",
        'format' => 'full_html',
      ],
    ]);
    $work_order->save();
    ...
    $fertilizing_fall_section->set('field_3rd_work_order', $work_order_id);
    $fertilizing_fall_section->save();

    // Display success message.
    $messenger->addMessage($this->t("<strong>Fall</strong> Fertilizing work order created for Contract #$contract_id."));

    // Implement the logic to create the Fall Fertilizing work order.
