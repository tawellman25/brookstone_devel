...
/**
 * Controller for creating "Grub Control" work orders.
 */
class GrubPreventionWorkOrderController extends ControllerBase {

  /**
   * Creates a "Grub Control" work order for a given contract.
   *
   ...
   */
  public function createGrubPreventionWorkOrder($contract_id) {
    ...
    // Load the referenced Grub Control entity.
    $grub_prevention_id = $contract->get('field_grub_prevention_on_lawn')->target_id;
    ...
    if (!$grub_prevention_section) {
      $messenger->addError('Referenced Grub Control Section entity not found.');
      return;
    }

    // Check if a work order already exists.
    if (!$grub_prevention_section->get('field_work_order')->isEmpty()) {
      $messenger->addError("A Grub Control Work Order already exists for this Contract #$contract_id.");
      return;
    }

    // Get the value of the 'field_estimate' field from the Grub Control entity & Extract the last number.
    ...
    // Get the value of the 'field_grub_prevention_season' field from the Pre-emergent entity.
    $contract_season = $grub_prevention_section->get('field_grub_prevention_season')->target_id;
    ...
    $work_order = $entity_type_manager->getStorage('work_order')->create([
      ...
      'field_work_todo_description' => [
        'value' => "<p>" . date('Y') . " - Grub Control as described</p>",
        'format' => 'full_html',
      ],
    ]);
    $work_order->save();
    ...
    $grub_prevention_section->set('field_work_order', $work_order_id);
    $grub_prevention_section->save();

    // Display success message.
    $messenger->addMessage($this->t("Grub Control work order created for Contract #$contract_id."));

    // Implement the logic to create the Grub Control work order.
