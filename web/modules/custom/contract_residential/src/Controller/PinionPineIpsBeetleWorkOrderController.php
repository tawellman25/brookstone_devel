...
/**
 * Controller for creating "Pinion Pine Ips Beetle" work orders.
 */
class PinionPineIpsBeetleWorkOrderController extends ControllerBase {

  /**
   * Creates a "Pinion Pine Ips Beetle" work order for a given contract.
   *
   ...
   */
  public function createPinionPineIpsBeetleWorkOrder($contract_id) {
    ...
    // Load the referenced Pinion Pine Ips Beetle entity.
    $pinion_pine_ips_beetle_id = $contract->get('field_ips_beetle_on_pinion_pine')->target_id;
    ...
    if (!$pinion_pine_ips_beetle_section) {
      $messenger->addError('Referenced Pinion Pine Ips Beetle Section entity not found.');
      return;
    }

    // Check if a work order already exists.
    if (!$pinion_pine_ips_beetle_section->get('field_work_order')->isEmpty()) {
      $messenger->addError("A Pinion Pine Ips Beetle Work Order already exists for this Contract #$contract_id.");
      return;
    }

    // Get the value of the 'field_estimate' field from the Pinion Pine Ips Beetle entity & Extract the last number.
    ...
    // Get the value of the 'field_pinion_pine_ips_beetle_season' field from the Pre-emergent entity.
    $contract_season = $pinion_pine_ips_beetle_section->get('field_pinion_pine_ips_beetle_season')->target_id;
    ...
    $work_order = $entity_type_manager->getStorage('work_order')->create([
      ...
      'field_work_todo_description' => [
        'value' => "<p>" . date('Y') . " - Pinion Pine Ips Beetle as described</p>",
        'format' => 'full_html',
      ],
    ]);
    $work_order->save();
    ...
    $pinion_pine_ips_beetle_section->set('field_work_order', $work_order_id);
    $pinion_pine_ips_beetle_section->save();

    // Display success message.
    $messenger->addMessage($this->t("Pinion Pine Ips Beetle work order created for Contract #$contract_id."));

    // Implement the logic to create the Pinion Pine Ips Beetle work order.
