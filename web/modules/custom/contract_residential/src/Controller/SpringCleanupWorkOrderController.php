...
/**
 * Controller for creating "Spring Cleanup" work orders.
 */
class SpringCleanupWorkOrderController extends ControllerBase {

  /**
   * Creates a "Spring Cleanup" work order for a given contract.
   *
   ...
   */
  public function createSpringCleanupWorkOrder($contract_id) {
    ...
    $spring_cleanup_id = $contract->get('field_spring_cleanup')->target_id;
    $spring_cleanup_section = $entity_type_manager->getStorage('contract_sections')->load($spring_cleanup_id);
    if (!$spring_cleanup_section) {
      $messenger->addError('Referenced Spring Cleanup Section entity not found.');
      return;
    }

    // Check if a work order already exists.
    if (!$spring_cleanup_section->get('field_work_order')->isEmpty()) {
    ...
    $spring_cleanup_section->set('field_work_order', $work_order_id);
    $spring_cleanup_section->save();

    // Display success message.
    $messenger->addMessage($this->t("Spring Cleanup work order created for Contract #$contract_id."));

    // Implement the logic to create the Spring Cleanup work order.
