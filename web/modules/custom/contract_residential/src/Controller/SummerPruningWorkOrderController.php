...
/**
 * Controller for creating "Summer Pruning" work orders.
 */
class SummerPruningWorkOrderController extends ControllerBase {

  /**
   * Creates a "Summer Pruning" work order for a given contract.
   *
   ...
   */
  public function createSummerPruningWorkOrder($contract_id) {
    ...
    $summer_pruning_id = $contract->get('field_summer_pruning')->target_id;
    $summer_pruning_section = $entity_type_manager->getStorage('contract_sections')->load($summer_pruning_id);
    if (!$summer_pruning_section) {
      $messenger->addError('Referenced Summer Pruning Section entity not found.');
      return;
    }
    ...
    // Implement the logic to create the Summer Pruning work order.
