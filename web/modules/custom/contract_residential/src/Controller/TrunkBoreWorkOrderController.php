...
/**
 * Controller for creating "Trunk Bore" work orders.
 */
class TrunkBoreWorkOrderController extends ControllerBase {

  /**
   * Creates a "Trunk Bore" work order for a given contract.
   *
   ...
   */
  public function createTrunkBoreWorkOrder($contract_id) {
    ...
    // Load the referenced Trunk Bore entity.
    $trunk_bore_id = $contract->get('field_trunk_bore_prevention')->target_id;
    $trunk_bore_section = $entity_type_manager->getStorage('contract_sections')->load($trunk_bore_id);
    if (!$trunk_bore_section) {
      $messenger->addError('Referenced Trunk Bore Section entity not found.');
      return;
    }

    // Check if a work order already exists.
    if (!$trunk_bore_section->get('field_work_order')->isEmpty()) {
      $messenger->addError("A Trunk Bore Work Order already exists for this Contract #$contract_id.");
      return;
    }

    // Get the value of the 'field_estimate' field from the Trunk Bore entity & Extract the last number.
    ...
    // Get the value of the 'field_trunk_bore_season' field from the Pre-emergent entity.
    $contract_season = $trunk_bore_section->get('field_trunk_bore_season')->target_id;
    ...
    $work_order = $entity_type_manager->getStorage('work_order')->create([
      ...
      'field_work_todo_description' => [
        'value' => "<p>" . date('Y') . " - Trunk Bore as described</p>",
        'format' => 'full_html',
      ],
    ]);
    $work_order->save();
    ...
    $trunk_bore_section->set('field_work_order', $work_order_id);
    $trunk_bore_section->save();

    // Display success message.
    $messenger->addMessage($this->t("Trunk Bore work order created for Contract #$contract_id."));

    // Implement the logic to create the Trunk Bore work order.
