...
/**
 * Creates a Deciduous Bore work order.
 *
 * @Action(
...
class CreateDeciduousBoreWorkOrderAction extends ViewsBulkOperationsActionBase {
  ...
  public function execute(EntityInterface $entity = NULL) {
    if ($entity && $entity->getEntityTypeId() === 'contracts') {
      ...
      // Load the referenced Deciduous Bored entity.
      $deciduous_bore_spray_id = $contract->get('field_deciduous_bore_treatment')->target_id;
      $deciduous_bore_spray_section = $entity_type_manager->getStorage('contract_sections')->load($deciduous_bore_spray_id);
      if (!$deciduous_bore_spray_section) {
        $messenger->addError('Referenced Deciduous Bore entity not found.');
        return;
      }

      // Check if a work order already exists.
      if (!$deciduous_bore_spray_section->get('field_work_order')->isEmpty()) {
        $messenger->addError("A Deciduous Bore Work Order already exists for this Contract #$contract_id.");
        return;
      }

      // Get the value of the 'field_estimate' field from the Deciduous Bored entity & Extract the last number.
      $estimate_text = $deciduous_bore_spray_section->get('field_estimate')->value;
      ...
      // Create a new Deciduous Bored Work Order.
      $work_order = $entity_type_manager->getStorage('work_order')->create([
        ...
        'field_work_todo_description' => [
          'value' => "<p>" . date('Y') . " - Spray Deciduous Bore as described</p>",
          'format' => 'full_html',
        ],
      ]);
      $work_order->save();
      ...
      $deciduous_bore_spray_section->set('field_work_order', $work_order_id);
      $deciduous_bore_spray_section->save();
