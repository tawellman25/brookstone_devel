 ...
 *
 * @Action(
 *   id = "create_deer_prevention_put_up_work_order_action",
 *   label = @Translation("Create PUT UP Deer Prevention Work Order"),
 *   category = @Translation("Custom"),
 *   confirm = TRUE,
 *   type = "contracts"
 * )
 */
class CreateDeerPreventionPutUpWorkOrderAction extends ViewsBulkOperationsActionBase {
  use MessengerTrait;
  ...
  public function execute(EntityInterface $entity = NULL) {
    if ($entity && $entity->getEntityTypeId() === 'contracts') {
      ...
      if (!$deer_prevention_section->get('field_work_order')->isEmpty()) {
        $messenger->addError("A PUT UP Deer Prevention Work Order already exists for this Contract #$contract_id.");
        return;
      }
      ...
      $work_order = $entity_type_manager->getStorage('work_order')->create([
        ...
        'field_status' => 1089,
        'field_invoiced' => 0,
        'field_deer_prevent_application' => 1275,
        'field_estimated_price' => $contract_estimate,
        'field_work_todo_description' => [
          'value' => "<p>" . date('Y') . " - <strong>Put Up</strong> Deer Prevention as described</p>",
          'format' => 'full_html',
        ],
      ]);
      $work_order->save();
