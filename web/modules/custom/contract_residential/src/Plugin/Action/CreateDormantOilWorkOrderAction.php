...
/**
 * Creates a Dormant Oil work order.
 *
 * @Action(
...
class CreateDormantOilWorkOrderAction extends ViewsBulkOperationsActionBase {
  ...
  public function execute(EntityInterface $entity = NULL) {
    if ($entity && $entity->getEntityTypeId() === 'contracts') {
      ...
      // Load the referenced Dormant Oild entity.
      $dormant_oil_spray_id = $contract->get('field_dormant_oil_spray')->target_id;
      $dormant_oil_spray_section = $entity_type_manager->getStorage('contract_sections')->load($dormant_oil_spray_id);
      if (!$dormant_oil_spray_section) {
        $messenger->addError('Referenced Dormant Oil entity not found.');
        return;
      }

      // Check if a work order already exists.
      if (!$dormant_oil_spray_section->get('field_work_order')->isEmpty()) {
        $messenger->addError("A Dormant Oil Work Order already exists for this Contract #$contract_id.");
        return;
      }

      // Get the value of the 'field_estimate' field from the Dormant Oild entity & Extract the last number.
      $estimate_text = $dormant_oil_spray_section->get('field_estimate')->value;
      ...
      // Create a new Dormant Oild Work Order.
      $work_order = $entity_type_manager->getStorage('work_order')->create([
        ...
        'field_work_todo_description' => [
          'value' => "<p>" . date('Y') . " - Spray Dormant Oil as described</p>",
          'format' => 'full_html',
        ],
      ]);
      $work_order->save();
      ...
      $dormant_oil_spray_section->set('field_work_order', $work_order_id);
      $dormant_oil_spray_section->save();
