...
/**
 * Creates a Grub Control work order.
 *
 * @Action(
 *   id = "create_grub_prevention_work_order_action",
 *   label = @Translation("Create Grub Controld Work Order"),
 *   category = @Translation("Custom"),
 *   confirm = TRUE
 * )
 */
class CreateGrubControlWorkOrderAction extends ViewsBulkOperationsActionBase {
  use MessengerTrait;
  ...
  public function execute(EntityInterface $entity = NULL) {
    if ($entity && $entity->getEntityTypeId() === 'contracts') {
      ...
      // Load the referenced Grub Controld entity.
      $grub_prevention_spray_id = $contract->get('field_grub_prevention_on_lawn')->target_id;
      $grub_prevention_spray_section = $entity_type_manager->getStorage('contract_sections')->load($grub_prevention_spray_id);
      if (!$grub_prevention_spray_section) {
        $messenger->addError('Referenced Grub Control entity not found.');
        return;
      }

      // Check if a work order already exists.
      if (!$grub_prevention_spray_section->get('field_work_order')->isEmpty()) {
        $messenger->addError("A Grub Control Work Order already exists for this Contract #$contract_id.");
        return;
      }

      // Get the value of the 'field_estimate' field from the Grub Controld entity & Extract the last number.
      $estimate_text = $grub_prevention_spray_section->get('field_estimate')->value;
      ...
      // Create a new Grub Controld Work Order.
      $work_order = $entity_type_manager->getStorage('work_order')->create([
        ...
        'field_work_todo_description' => [
          'value' => "<p>" . date('Y') . " - Spray Grub Control as described</p>",
          'format' => 'full_html',
        ],
      ]);
      $work_order->save();
      ...
      $grub_prevention_spray_section->set('field_work_order', $work_order_id);
      $grub_prevention_spray_section->save();

      // Display success message.
      $messenger->addMessage($this->t("Grub Control work order created for Contract #$contract_id."));
    }
  }
