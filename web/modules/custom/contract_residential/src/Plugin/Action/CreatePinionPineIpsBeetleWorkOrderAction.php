...
/**
 * Creates a Pinion Pine Ips Beetle work order.
 *
 * @Action(
 *   id = "create_pinion_pine_ips_beetle_work_order_action",
 *   label = @Translation("Create Pinion Pine Ips Beetled Work Order"),
 *   category = @Translation("Custom"),
 *   confirm = TRUE,
 *   type = "contracts"
 * )
 */
class CreatePinionPineIpsBeetleWorkOrderAction extends ViewsBulkOperationsActionBase {
  use MessengerTrait;
  ...
  public function execute(EntityInterface $entity = NULL) {
    if ($entity && $entity->getEntityTypeId() === 'contracts') {
      ...
      // Load the referenced Pinion Pine Ips Beetled entity.
      $pinion_pine_ips_beetle_spray_id = $contract->get('field_ips_beetle_on_pinion_pine')->target_id;
      $pinion_pine_ips_beetle_spray_section = $entity_type_manager->getStorage('contract_sections')->load($pinion_pine_ips_beetle_spray_id);
      if (!$pinion_pine_ips_beetle_spray_section) {
        $messenger->addError('Referenced Pinion Pine Ips Beetle entity not found.');
        return;
      }

      // Check if a work order already exists.
      if (!$pinion_pine_ips_beetle_spray_section->get('field_work_order')->isEmpty()) {
        $messenger->addError("A Pinion Pine Ips Beetle Work Order already exists for this Contract #$contract_id.");
        return;
      }

      // Get the value of the 'field_estimate' field from the Pinion Pine Ips Beetled entity & Extract the last number.
      $estimate_text = $pinion_pine_ips_beetle_spray_section->get('field_estimate')->value;
      ...
      // Create a new Pinion Pine Ips Beetled Work Order.
      $work_order = $entity_type_manager->getStorage('work_order')->create([
        ...
        'field_work_todo_description' => [
          'value' => "<p>" . date('Y') . " - Spray Pinion Pine Ips Beetle as described</p>",
          'format' => 'full_html',
        ],
      ]);
      $work_order->save();
      ...
      $pinion_pine_ips_beetle_spray_section->set('field_work_order', $work_order_id);
      $pinion_pine_ips_beetle_spray_section->save();

      // Display success message.
      $messenger->addMessage($this->t("Pinion Pine Ips Beetle work order created for Contract #$contract_id."));
    }
  }
