...
/**
 * Creates a Fall Pinyon Pine Ips Beetle work order.
 *
 * @Action(
 *   id = "create_pinyon_pine_ips_beetle_fall_work_order_action",
 *   label = @Translation("Create FALL Pinyon Pine Ips Beetle Work Order"),
 *   category = @Translation("Custom"),
 *   confirm = TRUE
 * )
 */
class CreatePinyonPineIpsBeetleFallWorkOrderAction extends ViewsBulkOperationsActionBase {
  use MessengerTrait;
  ...
  public function execute(EntityInterface $entity = NULL) {
    if ($entity && $entity->getEntityTypeId() === 'contracts') {
      ...
      // Check if a work order already exists for Fall.
      if (!$pinyon_pine_ips_beetle_spray_section->get('field_3rd_work_order')->isEmpty()) {
        $messenger->addError($this->t('A Fall Pinyon Pine Ips Beetle Work Order already exists for Contract #@id.', ['@id' => $contract_id]));
        return;
      }
      ...
      // Create a new Pinyon Pine Ips Beetle Work Order for Fall.
      $work_order = $entity_type_manager->getStorage('work_order')->create([
        ...
        'field_ips_beetle_season_ref' => 1102, // Fall
        ...
        'field_work_todo_description' => [
          'value' => $pinyon_pine_tree_count > 0
            ? strtr("<p>@year - <strong>Fall</strong> Spray Pinyon Pine Ips Beetle as described (for @count trees)</p>", [
                '@year' => date('Y'),
                '@count' => $pinyon_pine_tree_count,
              ])
            : strtr("<p>@year - <strong>Fall</strong> Spray Pinyon Pine Ips Beetle as described (Please Update Tree Count)</p>", [
                '@year' => date('Y'),
              ]),
          'format' => 'full_html',
        ],
      ...
      // Set the Fall work order reference in the contract section.
      $pinyon_pine_ips_beetle_spray_section->set('field_3rd_work_order', $work_order_id);
      $pinyon_pine_ips_beetle_spray_section->save();

      // Display success message.
      $messenger->addMessage($this->t('<strong>Fall</strong> Pinyon Pine Ips Beetle work order created for Contract #@id with @count trees.', [
        '@id' => $contract_id,
        '@count' => $pinyon_pine_tree_count,
      ]));
    }
