...
/**
 * Creates a Summer Pinyon Pine Ips Beetle work order.
 *
 * @Action(
 *   id = "create_pinyon_pine_ips_beetle_summer_work_order_action",
 *   label = @Translation("Create SUMMER Pinyon Pine Ips Beetle Work Order"),
 *   category = @Translation("Custom"),
 *   confirm = TRUE,
 *   type = "contracts"
 * )
 */
class CreatePinyonPineIpsBeetleSummerWorkOrderAction extends ViewsBulkOperationsActionBase {
  use MessengerTrait;
  ...
  public function execute(EntityInterface $entity = NULL) {
    if ($entity && $entity->getEntityTypeId() === 'contracts') {
      ...
      // Check if a work order already exists for Summer.
      if (!$pinyon_pine_ips_beetle_spray_section->get('field_2nd_work_order')->isEmpty()) {
        $messenger->addError($this->t('A Summer Pinyon Pine Ips Beetle Work Order already exists for Contract #@id.', ['@id' => $contract_id]));
        return;
      }
      ...
      // Check for existing work order for this property and Summer season.
      $existing_work_orders = $entity_type_manager->getStorage('work_order')->getQuery()
        ...
        ->condition('field_ips_beetle_season_ref', 1101) // Summer
        ->accessCheck(FALSE)
        ->count()
        ->execute();

      if ($existing_work_orders > 0) {
        $messenger->addError($this->t('A Summer Pinyon Pine Ips Beetle Work Order already exists for this property in Contract #@id.', ['@id' => $contract_id]));
        return;
      }
      ...
      // Create a new Pinyon Pine Ips Beetle Work Order for Summer.
      $work_order = $entity_type_manager->getStorage('work_order')->create([
        ...
        'field_ips_beetle_season_ref' => 1101, // Summer
        ...
        'field_work_todo_description' => [
          'value' => $pinyon_pine_tree_count > 0
            ? strtr("<p>@year - <strong>Summer</strong> Spray Pinyon Pine Ips Beetle as described (for @count trees)</p>", [
                '@year' => date('Y'),
                '@count' => $pinyon_pine_tree_count,
              ])
            : strtr("<p>@year - <strong>Summer</strong> Spray Pinyon Pine Ips Beetle as described (Please Update Tree Count)</p>", [
                '@year' => date('Y'),
              ]),
          'format' => 'full_html',
        ],
      ...
      // Set the Summer work order reference in the contract section.
      $pinyon_pine_ips_beetle_spray_section->set('field_2nd_work_order', $work_order_id);
      $pinyon_pine_ips_beetle_spray_section->save();

      // Display success message.
      $messenger->addMessage($this->t('<strong>Summer</strong> Pinyon Pine Ips Beetle work order created for Contract #@id with @count trees.', [
        '@id' => $contract_id,
        '@count' => $pinyon_pine_tree_count,
      ]));
    }
