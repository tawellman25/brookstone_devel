...
class CreateSprinklerWinterizingWorkOrderAction extends ViewsBulkOperationsActionBase {
  ...
  public function execute(EntityInterface $entity = NULL) {
    if ($entity && $entity->getEntityTypeId() === 'contracts') {
      ...
      // Load the referenced Sprinkler Winterizing entity.
      $sprinkler_winterizing_id = $contract->get('field_irrigation_shut_down')->target_id;
      ...
      if (!$sprinkler_winterizing_section) {
        $messenger->addError('Referenced Winterizing entity not found.');
        return;
      }

      // Check if a work order already exists.
      if (!$sprinkler_winterizing_section->get('field_work_order')->isEmpty()) {
        $messenger->addError("A Winterizing Work Order already exists for this Contract #$contract_id.");
        return;
      }
      ...
      $sprinkler_winterizing_section->set('field_work_order', $work_order_id);
      $sprinkler_winterizing_section->save();

      // Display success message.
      $messenger->addMessage($this->t("Sprinkler System Winterizing work order created for Contract #$contract_id."));
    }
  }
