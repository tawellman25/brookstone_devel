...
/**
 * Creates a Trunk Bore work order.
 *
 * @Action(
...
class CreateTrunkBoreWorkOrderAction extends ViewsBulkOperationsActionBase {
  ...
  public function execute(EntityInterface $entity = NULL) {
    if ($entity && $entity->getEntityTypeId() === 'contracts') {
      ...
      if (!$trunk_bore_spray_section->get('field_work_order')->isEmpty()) {
        $messenger->addError("A Trunk Bore Work Order already exists for this Contract #$contract_id.");
        return;
      }

      // Get the value of the 'field_estimate' field from the Trunk Bored entity & Extract the last number.
      $estimate_text = $trunk_bore_spray_section->get('field_estimate')->value;
      ...
      // Create a new Trunk Bored Work Order.
      $work_order = $entity_type_manager->getStorage('work_order')->create([
        ...
        'field_work_todo_description' => [
          'value' => "<p>" . date('Y') . " - Spray Trunk Bore as described</p>",
          'format' => 'full_html',
        ],
      ]);
      $work_order->save();
      ...
      $trunk_bore_spray_section->set('field_work_order', $work_order_id);
      $trunk_bore_spray_section->save();
