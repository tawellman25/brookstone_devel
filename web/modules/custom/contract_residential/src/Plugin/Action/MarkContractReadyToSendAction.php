...
/**
 * Mark Contract as Ready to Send (contract_status = 1118) with guardrails.
 * Admin override: role "administrator" may bypass guardrails.
 *
 * @Action(
 *   id = "contract_residential_mark_ready_to_send",
 *   label = @Translation("Mark Contract: Ready to Send"),
 *   category = @Translation("Contracts"),
 *   confirm = TRUE
 * )
 */
class MarkContractReadyToSendAction extends ViewsBulkOperationsActionBase {
  use MessengerTrait;
  ...
  public function execute(EntityInterface $entity = NULL) {
    ...
    $action_key = 'contract_residential_mark_ready_to_send';
    $target_tid = 1118; // Ready to Send
    ...
    if (!$is_admin_override) {
      // Allow from: Created - Updating, On Hold.
      $allowed_from = [
        1117, // Created - Updating
        1126, // On Hold (optional)
      ];

      // Disallow from later stages.
      $disallowed_from = [
        1119, // Sent - Posted
        1120, // Client Viewed
        1121, // Received Back
      ...
      if (in_array($current_tid, $disallowed_from, TRUE)) {
        $this->messenger()->addError($this->t(
          '@label cannot be marked Ready to Send because it is already past that stage. Current status TID: @current.',
      ...
      if (!in_array($current_tid, $allowed_from, TRUE)) {
        $this->messenger()->addError($this->t(
          '@label cannot be marked Ready to Send from the current status (TID: @current). Allowed from: Created - Updating, On Hold.',
          ['@label' => $label, '@current' => $current_tid ?: 'none']
        ));
        return;
      }
    ...
    if ($is_admin_override) {
      $this->messenger()->addWarning($this->t('@label marked Ready to Send (administrator override).', ['@label' => $label]));
      return;
    }

    $this->messenger()->addMessage($this->t('@label marked Ready to Send.', ['@label' => $label]));
  }
