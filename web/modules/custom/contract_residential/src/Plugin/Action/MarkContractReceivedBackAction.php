...
/**
 * Mark Contract as Received Back (contract_status = 1121) with guardrails.
 * Admin override: role "administrator" may bypass guardrails.
 *
 * @Action(
 *   id = "contract_residential_mark_received_back",
 *   label = @Translation("Mark Contract: Received Back"),
 *   category = @Translation("Contracts"),
 *   confirm = TRUE
 * )
 */
class MarkContractReceivedBackAction extends ViewsBulkOperationsActionBase {
  use MessengerTrait;
  ...
  public function execute(EntityInterface $entity = NULL) {
    ...
    $action_key = 'contract_residential_mark_received_back';
    $target_tid = 1121; // Received Back
    ...
    if (!$is_admin_override) {
      ...
      if (in_array($current_tid, $disallowed_from, TRUE)) {
        $this->messenger()->addError($this->t(
          '@label cannot be marked as Received Back because it is already past that stage. Current status TID: @current.',
      ...
      if (!in_array($current_tid, $allowed_from, TRUE)) {
        $this->messenger()->addError($this->t(
          '@label cannot be marked as Received Back from the current status (TID: @current). Allowed from: Ready to Send, Sent - Posted, Client Viewed, On Hold.',
          ['@label' => $label, '@current' => $current_tid ?: 'none']
        ));
        return;
      }
    ...
    if ($is_admin_override) {
      $this->messenger()->addWarning($this->t('@label marked as Received Back (administrator override).', [
        '@label' => $label,
      ]));
      return;
    }

    $this->messenger()->addMessage($this->t('@label marked as Received Back.', [
      '@label' => $label,
    ]));
  }
