...
/**
 * Mark Contract as Sent - Posted (contract_status = 1119) with guardrails.
 * Admin override: role "administrator" may bypass guardrails.
 *
 * @Action(
 *   id = "contract_residential_mark_sent_posted",
 *   label = @Translation("Mark Contract: Sent - Posted"),
 *   category = @Translation("Contracts"),
 *   confirm = TRUE
 * )
 */
class MarkContractSentPostedAction extends ViewsBulkOperationsActionBase {
  use MessengerTrait;
  ...
  public function execute(EntityInterface $entity = NULL) {
    ...
    $action_key = 'contract_residential_mark_sent_posted';
    $target_tid = 1119; // Sent - Posted
    ...
    if (!$is_admin_override) {
      $allowed_from = [
        1117, // Created - Updating (optional)
        1118, // Ready to Send
      ...
      if (in_array($current_tid, $disallowed_from, TRUE)) {
        $this->messenger()->addError($this->t(
          '@label cannot be marked as Sent - Posted because it is already past that stage. Current status TID: @current.',
      ...
      if (!in_array($current_tid, $allowed_from, TRUE)) {
        $this->messenger()->addError($this->t(
          '@label cannot be marked as Sent - Posted from the current status (TID: @current). Allowed from: Created - Updating, Ready to Send, On Hold.',
          ['@label' => $label, '@current' => $current_tid ?: 'none']
        ));
        return;
      }
    ...
    if ($is_admin_override) {
      $this->messenger()->addWarning($this->t('@label marked as Sent - Posted (administrator override).', [
        '@label' => $label,
      ]));
      return;
    }

    $this->messenger()->addMessage($this->t('@label marked as Sent - Posted.', [
      '@label' => $label,
    ]));
  }
