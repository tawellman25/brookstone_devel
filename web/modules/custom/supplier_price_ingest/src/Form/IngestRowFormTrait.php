...
trait IngestRowFormTrait {
  ...
  protected function nextRowRedirect(
    ...
    // Defensive: strip ?destination= from the current request so the
    // Url we return actually wins. Drupal's form-redirect logic
    // honors ?destination= over $form_state->setRedirectUrl() — if
    // the operation link was generated by a Views entity_operations
    // field with destination: true (the default), every submit
    // bounces back to the queue page instead of the next row. The
    // view configs now set destination: false on the Operations
    // column, but this strip is the belt-and-suspenders defense
    // against manually-pasted URLs and any other source of the
    // query param.
