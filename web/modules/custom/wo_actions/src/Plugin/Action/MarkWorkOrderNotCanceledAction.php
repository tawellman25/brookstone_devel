...
/**
 * Marks Work Order as NOT Canceled.
 *
 * @Action(
 *   id = "mark_work_order_not_canceled_action",
 *   label = @Translation("Mark WO NOT Canceled"),
 *   category = @Translation("Custom"),
 *   confirm = TRUE
 * )
 */
class MarkWorkOrderNotCanceledAction extends ViewsBulkOperationsActionBase {
  use MessengerTrait;
    ...
    if ($entity && $entity->getEntityTypeId() === 'work_order') {
      ...
      $wo_status_updates = \Drupal::entityTypeManager()
        ...
        ->create([
          ...
          'field_status' => 1096,
          'field_status_change_note' => "$currentUserName marked this Work Order as NOT Canceled.",
          'uid' => $currentUserId,
        ]);

      $wo_status_updates->save();

      $messenger->addMessage("The Work Order #$workorder_id has been Marked as NOT Canceled.");
    }
  }
