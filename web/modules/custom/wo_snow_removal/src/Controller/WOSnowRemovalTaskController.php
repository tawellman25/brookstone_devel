...
namespace Drupal\wo_snow_removal\Controller;
...
class WOSnowRemovalTaskController extends ControllerBase {
  ...
  public function startWorkflow($property_id) {
    ...
    $query->fields('wtlfd', ['id'])
        ...
        ->condition('wtlfd.type', 'snow_removal')
    ...
    if ($result) {
        ...
        // Create a new Snow Removal Work Order if no incomplete tasks are found.
        $work_order = $this->entityTypeManager->getStorage('work_order')->create([
        'type' => 'snow_removal',
        'uid' => $current_user_id,
        'created' => time(),
        'field_invoiced' => 0,
        'field_work_todo_description' => "$year - Snow Removal as needed",
  ...
  public function customAccessCheck(AccountInterface $account, Request $request) {
    ...
    \Drupal::logger('wo_snow_removal')->debug('Current user ID: ' . $account->id());

    // Check if the user has the necessary permission.
    if ($account->hasPermission('create work_order entities')) {
        \Drupal::logger('wo_snow_removal')->debug('User has permission: Create new Work Order entities');
        return AccessResult::allowed();
    } else {
        \Drupal::logger('wo_snow_removal')->debug('User does not have permission: Create new Work Order entities');
    }

    // If access is denied, log a message.
    \Drupal::logger('wo_snow_removal')->debug('Access denied for user: ' . $account->getAccountName());
