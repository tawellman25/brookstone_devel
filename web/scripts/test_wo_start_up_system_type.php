...
$svc = (int) reset($etm->getStorage('taxonomy_term')->getQuery()
    ->condition('field_service_bundle', 'sprinkler_start_up')->accessCheck(FALSE)->range(0, 1)->execute());
...
$wo1 = $etm->getStorage("work_order")->create(['type' => 'sprinkler_start_up', 'field_property' => $dirtyPid, 'field_service' => $svc, 'field_status' => 1092]);
$wo1->save();
$made[] = $wo1->id();
...
$ok("field_task_rate = 105 (80 base + 25 pump), got $rate", abs($rate - 105.0) < 0.01);
...
$wo2 = $etm->getStorage("work_order")->create(['type' => 'sprinkler_start_up', 'field_property' => $domPid, 'field_service' => $svc, 'field_status' => 1092]);
$wo2->save();
$made[] = $wo2->id();
...
$ok("WO value wins → task_rate = 105, got $rate2 (start-up)", abs($rate2 - 105.0) < 0.01);
