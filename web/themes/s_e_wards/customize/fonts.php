...
function s_e_wards_render_option_font(){
    $fonts = s_e_wards_fonts();
    $output = '';
...
function s_e_wards_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function s_e_wards_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(s_e_wards_fonts())) > 17){
...
function s_e_wards_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode((string)$json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= s_e_wards_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= s_e_wards_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

function s_e_wards_options_patterns(){
    $output = '';
    $file_path = \Drupal::service('extension.list.theme')->getPath('s_e_wards');
